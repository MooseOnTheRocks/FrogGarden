...
import static dev.foltz.froggarden.FrogGarden.EPSILON;
import static dev.foltz.froggarden.util.MathUtil.*;
...
class Box {
    ...
    Hit intersectSegment(PVector pos, PVector delta, float paddingX, float paddingY) {
        ...
        final float nearTime = max(nearTimeX, nearTimeY);
        final float farTime = min(farTimeX, farTimeY);
    ...
    Hit intersectPoint(PVector point) {
        ...
        final float px = half.x - Math.abs(dx);
        if (px <= 0) {
            return null;
        }
